...
class CategoryController extends Controller
{
    ...
    public function delete(Request $request,$id){
        $arr= Category::pluck('parent_category_id')->toArray();
        $model=Category::where('id', $id)->get();
        // dd($model[0]->id);
        // die;
        if (in_array($model[0]->id, $arr)) {
            $request->session()->flash('err_message','Category cant be deleted because it has sub category.');
            return redirect('admin/category');
        } else {
            $model2=Category::find($id);
            $model2->delete();
